...
import { footerApi, type FooterInfo, type FooterLink } from '@/lib/apis/footer.api';
...
const DEFAULT_BRAND_DESCRIPTION =
  'Nền tảng chia sẻ video hàng đầu Việt Nam. Khám phá hàng triệu nội dung sáng tạo mỗi ngày.';
const DEFAULT_COPYRIGHT = 'snha. All rights reserved.';

const SOCIAL_ICONS: Record<string, string> = {
  facebook:
    'M22 12.06C22 6.5 17.52 2 12 2S2 6.5 2 12.06c0 5 3.66 9.15 8.44 9.94v-7.03H7.9v-2.9h2.54V9.85c0-2.52 1.49-3.91 3.78-3.91 1.1 0 2.24.2 2.24.2v2.46h-1.26c-1.24 0-1.63.78-1.63 1.57v1.88h2.78l-.44 2.9h-2.34V22c4.78-.79 8.44-4.94 8.44-9.94Z',
  youtube:
    'M23.5 6.2a3 3 0 0 0-2.12-2.12C19.5 3.6 12 3.6 12 3.6s-7.5 0-9.38.48A3 3 0 0 0 .5 6.2 31.3 31.3 0 0 0 0 12a31.3 31.3 0 0 0 .5 5.8 3 3 0 0 0 2.12 2.12C4.5 20.4 12 20.4 12 20.4s7.5 0 9.38-.48a3 3 0 0 0 2.12-2.12A31.3 31.3 0 0 0 24 12a31.3 31.3 0 0 0-.5-5.8ZM9.6 15.6V8.4l6.2 3.6-6.2 3.6Z',
  tiktok:
    'M16.5 3c.3 2.1 1.5 3.9 3.5 4.2v3.1c-1.3.1-2.5-.3-3.6-.9v6.3a5.6 5.6 0 1 1-5.6-5.6c.3 0 .6 0 .9.1v3.2a2.5 2.5 0 1 0 1.7 2.4V3h3.1Z',
  instagram:
...
type SocialItem = { name: string; href: string; path?: string };
...
function toSimpleLinks(links: FooterLink[] | undefined, fallback: SimpleLink[]): SimpleLink[] {
  return links && links.length > 0 ? links : fallback;
}

function toSocials(links: FooterLink[] | undefined): SocialItem[] {
  if (links && links.length > 0) {
    return links.map((l) => ({
      name: l.label,
      href: l.href,
      path: SOCIAL_ICONS[l.label.toLowerCase()],
    }));
  }
  return Object.entries(SOCIAL_ICONS).map(([name, path]) => ({
...
export default function Footer() {
  ...
  const links: FooterInfo['links'] = info?.links ?? ({} as FooterInfo['links']);

  const brandDescription = settings.brand_description || DEFAULT_BRAND_DESCRIPTION;
  const copyright = settings.copyright_text || DEFAULT_COPYRIGHT;
  const quickLinks = toSimpleLinks(links.QUICK_LINKS, DEFAULT_LINKS.QUICK_LINKS);
  const categories = toSimpleLinks(links.CATEGORIES, DEFAULT_LINKS.CATEGORIES);
  const supportLinks = toSimpleLinks(links.SUPPORT, DEFAULT_LINKS.SUPPORT);
  const bottomLinks = toSimpleLinks(links.BOTTOM, DEFAULT_LINKS.BOTTOM);
  const socials = toSocials(links.SOCIAL);
