...
/**
 * Lưới video công khai dùng chung cho /category/{slug} và /tag/{tag}:
 * - Sort chips + phân trang numbered giống trang chủ (Pagination dùng chung).
 * - ?page=N và ?sort= đồng bộ lên URL để share/lưu lại được.
 * - Lọc phía backend qua categorySlug hoặc tag (truyền từ page cha).
 */
...
interface PublicVideoListProps {
    /** Tiêu đề trang */
    heading: string;
    /** Phụ đề hiển thị dưới tiêu đề (tuỳ chọn) */
    subtitle?: string;
    /** Lọc theo slug category (truyền 1 trong 2) */
    categorySlug?: string;
    /** Lọc theo tag (truyền 1 trong 2) */
    tag?: string;
...
export default function PublicVideoList({
    ...
    // Lần nạp đầu: dùng page từ URL, các lần đổi sort sau đó reset về trang 0
    const isFirstLoadRef = useRef(true);

    // Đổi filter (slug/tag) thì nạp lại từ đầu
    useEffect(() => {
    ...
    const loadPage = useCallback(
        async (pageToLoad: number) => {
            ...
            try {
                const data = await videoApi.getPublicVideos(pageToLoad, PAGE_SIZE, sortBy, categorySlug, tag);
    ...
    return (
        <div className='container mx-auto px-4 py-8'>
            ...
            {loading ? (
                ...
                <div className='text-center py-16'>
                    <p className='text-foreground opacity-70 mb-4'>{emptyMessage}</p>
